...
def create_app(config_class=Config):
    ...
    def unauthorized_error(e):
        return jsonify(
            {"error": "Unauthorized", "message": "No autorizado o token no válido"}
        ), 401

    @app.errorhandler(403)
    def forbidden_error(e):
        return jsonify(
            {"error": "Forbidden", "message": "Acceso denegado: permisos insuficientes"}
        ), 403

    @app.errorhandler(404)
    def not_found_error(e):
        return jsonify(
            {"error": "Not Found", "message": "El recurso solicitado no fue encontrado"}
        ), 404
